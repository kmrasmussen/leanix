...
def renderWithPolicyToFile (policy : Leanix.EscapePolicy) (flake : Leanix.Flake)
    (outputPath : String) : IO UInt32 := do
  match Leanix.validateFlakeWithPolicy policy flake with
  | .ok _ => renderToFile flake outputPath
  | .error error =>
      IO.eprintln s!"error: {error}"
      pure 1
...
def main (args : List String) : IO UInt32 := do
  ...
  | ["render-invalid-build-plan-parent-path", "--out", outputPath] =>
      renderExceptToFile Leanix.Examples.parentTraversalBuildPlanFlake outputPath
  | ["render-invalid-build-plan-absolute-destination", "--out", outputPath] =>
      renderExceptToFile Leanix.Examples.absoluteDestinationBuildPlanFlake outputPath
  ...
  | ["render-invalid-ci-impure-source", "--out", outputPath] =>
      renderWithPolicyToFile .ci Leanix.Examples.impureLocalSourceFlake outputPath
